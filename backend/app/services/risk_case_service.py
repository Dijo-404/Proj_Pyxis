...
class RiskCaseService:
    ...
    def evaluate_transaction(self, request: TransactionEvaluateRequest) -> RiskCaseResponse:
        ...
        comparisons, critical_evidence = self.scenario_service.compare(
            investigation.scenarios, transaction, anomaly
        )
        current_risk_score = min(
            100,
            max(
                anomaly.score,
                round(max((item.match_score for item in comparisons), default=0) * 100),
            ),
        )
    ...
    def investigate_case(self, case_id: str) -> RiskCaseResponse | None:
        ...
        if transaction is None:
            return existing
        investigation = self.investigation_service.investigate(
            transaction, existing.financial_twin, existing.anomaly_assessment
        )
        comparisons, critical_evidence = self.scenario_service.compare(
            investigation.scenarios, transaction, existing.anomaly_assessment
        )
