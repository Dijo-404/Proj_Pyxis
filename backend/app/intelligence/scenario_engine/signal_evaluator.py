...
def evaluate_signal(signal: ExpectedSignal, transaction: TransactionInput, anomaly: AnomalyAssessment) -> SignalEvaluation:
    ...
    elif name in {"new_beneficiaries", "verified_supplier_relationships", "missing_supplier_verification"}:
        status = "UNKNOWN" if name.startswith("verified") else ("MATCH" if features.get("is_new_beneficiary") else "CONTRADICT")
    ...
    return SignalEvaluation(signal=name, status=status, weight=signal.weight, observed_value=observed, explanation=f"{name} evaluated as {status.lower().replace('_', ' ')}.")
