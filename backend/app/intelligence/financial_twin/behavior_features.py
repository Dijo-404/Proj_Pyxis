...
def build_behavior_profile(
    customer_id: str, transactions: list[TransactionInput]
) -> BehaviorProfile:
    if not transactions:
    ...
    beneficiaries = [
        transaction.beneficiary_id for transaction in transactions if transaction.beneficiary_id
    ]
    ...
    international_count = sum(
        1 for country in countries if home_country and country != home_country
    )
