    ...
    if transaction.amount > p95 * 3:
        rules.append(
            TriggeredRule(
                ...
                severity="HIGH",
                description="Amount exceeds customer p95 by configured multiplier.",
            )
        )
    if (
        ...
        rules.append(
            TriggeredRule(
                ...
                severity="MEDIUM",
                description="Transaction uses a beneficiary outside the trusted profile.",
            )
        )
    if transaction.country not in twin.geography_profile.common_countries:
        rules.append(
            TriggeredRule(
                rule_id="new_country",
                severity="MEDIUM",
                description="Transaction country is outside the usual geography profile.",
            )
        )
    if hourly_velocity > max(twin.velocity_profile.maximum_normal_hourly_count, 1):
        rules.append(
            TriggeredRule(
                rule_id="high_hourly_velocity",
                severity="HIGH",
                description="Hourly transaction count exceeds normal customer velocity.",
            )
        )
    if not (
        ...
        rules.append(
            TriggeredRule(
                ...
                severity="LOW",
                description="Transaction occurred outside usual active hours.",
            )
        )
