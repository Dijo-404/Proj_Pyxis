...
class FinancialTwin(BehaviorProfile):
    ...
    def demo_baseline(cls, customer_id: str) -> "FinancialTwin":
        return cls(
            ...
            version=1,
            amount_profile=AmountProfile(median=42000, p95=125000, trusted_min=10000, trusted_max=150000),
            velocity_profile=VelocityProfile(average_transactions_per_day=3.2, maximum_normal_hourly_count=4),
            beneficiary_profile=BeneficiaryProfile(known_beneficiaries=8, known_beneficiary_ratio=0.92, known_beneficiary_ids=["BEN-001", "BEN-002", "BEN-003"]),
            geography_profile=GeographyProfile(common_countries=["India"], international_transfer_frequency=0.03),
            time_profile=TimeProfile(usual_start_hour=8, usual_end_hour=21),
            business_profile=BusinessProfile(business_type="Textile Retail", expected_monthly_turnover=700000),
        )
